...
namespace WebRTCServer
{
    ...
    public class WebRTCSessionContext
    {
        private readonly ConcurrentDictionary<String,WebRTCSessionData> _list = new ConcurrentDictionary<string, WebRTCSessionData>();
        ...
        {
            var sessionData = _list[sessionId];
        ...
        public WebRTCSessionData OpenSession()
        {
            ...
            if (!_list.TryAdd(id,sessionData))
            {
        ...
        {
            if (!_list.TryRemove(sessionId,out WebRTCSessionData sessionData))
            {
        ...
        {
            if (!_list.TryGetValue(sessionId,out WebRTCSessionData sessionData))
            {
