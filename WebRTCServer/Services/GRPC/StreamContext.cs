...
namespace WebRTCServer
{
    public class StreamContext
    {
        ...
        public async Task<bool> Send<T>(string id,T response)
            where T:IMessage,new() 
        {
            ...
            if (serverid == 0)
            {
                if (_peerList.TryGetValue(peerid, out Tuple<SemaphoreSlim, object, CancellationTokenSource> stream))
                {
            ...
            {
                return await Forward(serverid,id,response);
            }
        }
